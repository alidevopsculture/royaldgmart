...
export default function WomenStitchedSuits() {
  ...
  const fetchProducts = async (pageNum: number) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/products?category=WOMEN STICHED SUITS&page=${pageNum}&limit=20`);
    ...
    <>
      ...
      <section className="relative w-full h-[520px] md:h-[600px] flex items-center justify-center mb-8 overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center z-10">
          ...
          <p className="text-lg md:text-xl text-white font-medium mb-6" style={{ fontFamily: 'Quicksand, sans-serif' }}>Ready-to-Wear Elegant Suits</p>
        </div>
      </section>
          ...
          <ol className="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
              <div className="flex items-center">
                ...
                <span className="ml-1 text-sm font-medium text-gray-500 md:ml-2">Women Stitched Suits</span>
              </div>
            </li>
          </ol>
        </nav>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 lg:gap-8">
            {products && products.length > 0 ? (
              ...
              <div className="col-span-full text-center py-12">
                <p className="text-gray-500 text-lg">No stitched suits found</p>
              </div>
            )}
          </div>
          {loadingMore && (
