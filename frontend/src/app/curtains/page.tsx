...
export default function Curtains() {
  ...
  const fetchProducts = async (pageNum: number) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/products?category=CURTAINS&page=${pageNum}&limit=20`);
  ...
  return (
    <>
      ...
      <section className="relative w-full h-[520px] md:h-[600px] flex items-center justify-center mb-8 overflow-hidden">
        <Image src="/banners/landscape-15.png" alt="Curtains Banner" fill priority className="object-cover opacity-100" />
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center z-10">
          <h1 className="text-4xl md:text-5xl font-extrabold text-white drop-shadow mb-4">Beautiful Curtains Collection</h1>
          <p className="text-lg md:text-xl text-white font-medium mb-6">Style & Privacy for Your Home</p>
        </div>
      </section>
          ...
          <ol className="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
              <div className="flex items-center">
                ...
                <span className="ml-1 text-sm font-medium text-gray-500 md:ml-2">Curtains</span>
              </div>
            </li>
          </ol>
        </nav>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 lg:gap-8">
            {products && products.length > 0 ? (
              ...
              <div className="col-span-full text-center py-12">
                <p className="text-gray-500 text-lg">No curtains found</p>
              </div>
            )}
          </div>
          {loadingMore && (
