...
export function Returns() {
  ...
  useEffect(() => {
    ...
    // Listen for return updates from user actions
    const syncInterval = setInterval(() => {
      const syncData = localStorage.getItem('adminOrdersSync')
      if (syncData) {
        const { timestamp, orderId, status, reason, type } = JSON.parse(syncData)
        if (Date.now() - timestamp < 5000 && type === 'regular' && status === 'return_initiated') {
          // Refresh the returns list when new return is initiated
          fetchReturns()
    ...
    // Listen for direct return request events
    const handleReturnRequest = () => {
      setTimeout(() => fetchReturns(), 1000) // Small delay to ensure backend update
    }
    
    window.addEventListener('returnRequestSubmitted', handleReturnRequest)
    
    return () => {
      clearInterval(syncInterval)
      window.removeEventListener('returnRequestSubmitted', handleReturnRequest)
    }
  }, [])
  ...
  const fetchReturns = async () => {
    try {
      ...
      const returnOrders = Array.isArray(allOrders) ? allOrders.filter(order => {
        const hasReturnStatus = order.status === 'return_initiated' || order.status === 'return_approved' || order.status === 'returned'
        const hasReturnReason = order.returnReason
        console.log(`Order ${order._id}: status=${order.status}, returnReason=${order.returnReason}, hasReturnStatus=${hasReturnStatus}`)
        return hasReturnStatus || hasReturnReason
      }) : []
      console.log('Filtered return orders:', returnOrders)
  ...
  const handleStatusUpdate = async (orderId: string, status: string) => {
    try {
      ...
      if (result.success) {
        ...
        toast.success('Return status updated and email sent')
      }
    } catch (error) {
      ...
      <div className="bg-white border-b border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">Manage regular order returns</p>
          </div>
